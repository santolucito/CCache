...
#elif (UINT_MAX >= 0xffffffffL)		/* 32 bit or more */
...
int load_file ( const char *file_name, lzo_uint *len )
{
	FILE *f;
	long ll = -1;
	unsigned long ul;
...
#ifdef USE_CORPUS
	                  "Ab::c:d:FhHLm::n:OqQs:STvV@123456789",
#else
	                  "Ab::c:d:FhHLm::n:OqQSTvV@123456789",
#endif
	                  longopts, (int *)0)) >= 0)
	{
		if (do_option(optc) != 0)
