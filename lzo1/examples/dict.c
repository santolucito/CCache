 ...
 */
	...
	out = lzo_malloc(in_len + in_len / 64 + 16 + 3);
	new = lzo_malloc(in_len);
	wrkmem = lzo_malloc(LZO1X_999_MEM_COMPRESS);
	if (in == NULL || out == NULL || new == NULL || wrkmem == NULL)
	{
...
/*
 * Step 4: decompress again, now going from `out' to `new'
 */
 	new_len = in_len;
	r = lzo1x_decompress_dict_safe(out,out_len,new,&new_len,NULL,dict,dict_len);
	if (r != LZO_E_OK)
	{
		/* this should NEVER happen */
 ...
 */
	if (new_len != in_len || lzo_memcmp(in,new,in_len) != 0)
	{
		/* this should NEVER happen */
